...
SSL_CTX* loadAndVerify(const char* CertFile, const char* KeyFile) {
   // create ctx
    ...
    // verift CA
    SSL_CTX_load_verify_locations(ctx, "key/cacert.pem", NULL);
...
SSL_CTX* initClientCTX(int& file) {
    ...
    char mcert[20] = {0};
    char mkey[20] = {0};
    if (strcmp(c, "1") == 0) { file = 1; strcpy(mcert, "key/cliCert.pem"); strcpy(mkey, "key/cliKey.pem"); }
    else                     { file = 2; strcpy(mcert, "key/cli2Cert.pem"); strcpy(mkey, "key/cli2Key.pem"); }
